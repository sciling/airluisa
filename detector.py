...
class Detector():
    ...
    def __init__(self, model='yolox-s', ckpt='yolox_s.pth.tar', use_cude=True):
        ...
        self.device = torch.device('cuda:0') if use_cude and torch.cuda.is_available() else torch.device('cpu')
    ...
    def detect(self, raw_img, visual=True, conf=0.5):
        ...
        with torch.no_grad():
            outputs = self.model(img).cuda() if self.device == torch.device('cuda:0') else self.model(img)
            outputs = postprocess(
                outputs, self.exp.num_classes, self.exp.test_conf, self.exp.nmsthre)#[0].cpu().numpy() # TODO:用户可更改
